...
def update(model, experience, gamma, experience_replay, n_actions=2, n_samples=4, target_model=None):
...
def play_one_episode(session, environment, epsilon, gamma=0.99, max_steps=10000, experience_replay=True,
    ...
    if use_dual_model:
        target_model = MountainCarNeuralNetwork(session, n_actions, environment)
    else:
        target_model = model
    ...
    while not done and time_step < max_steps:
        ...
        if use_dual_model and time_step % 100 == 0:
            update(target_model, experience, gamma, experience_replay, n_actions=n_actions, n_samples=n_samples)
        if done:
            break
